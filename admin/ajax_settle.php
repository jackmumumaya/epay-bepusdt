...
case 'settle_info':
	...
	$usdt_chain_select = '<option value="">不使用USDT结算</option>';
	foreach($usdt_chains as $key => $value){
	    $selected = ($rows['usdt_chain'] == $key) ? ' selected' : '';
	    $usdt_chain_select .= '<option value="'.$key.'"'.$selected.'>'.$value.'</option>';
	}
	
	$data = '<div class="form-group"><div class="input-group"><div class="input-group-addon">结算方式</div><select class="form-control" id="pay_type" default="'.$rows['type'].'">'.($conf['settle_alipay']?'<option value="1">支付宝</option>':null).''.($conf['settle_wxpay']?'<option value="2">微信</option>':null).''.($conf['settle_qqpay']?'<option value="3">QQ钱包</option>':null).''.($conf['settle_bank']?'<option value="4">银行卡</option>':null).''.($conf['transfer_usdt']?'<option value="5">USDT/USDC</option>':null).'</select></div></div>';
	$data .= '<div class="form-group"><div class="input-group"><div class="input-group-addon">USDT结算链路</div><select class="form-control" id="usdt_chain">'.$usdt_chain_select.'</select></div></div>';
...
case 'settle_save':
	...
	$usdt_chain=trim($_POST['usdt_chain']);
	$pay_account=htmlspecialchars(trim($_POST['pay_account']));
	$pay_name=htmlspecialchars(trim($_POST['pay_name']));
	$data = ['type'=>$pay_type, 'usdt_chain'=>$usdt_chain, 'account'=>$pay_account, 'username'=>$pay_name];
...
case 'transfer':
	...
	if($type == 5){
		$bizParam = [
			...
			'payee_real_name' => $row['username'],
			'money' => $row['realmoney'],
			'transfer_desc' => $conf['transfer_desc'],
			'usdt_chain' => $row['usdt_chain'],
		];
		$result = \lib\Plugin::call('transfer', $channel, $bizParam);
	}else{
